...
class MultilayerPerceptron:
    ...
    def __init__(self, data, labels, layers, epsilon, normalize_data=False):
        ...
        data_processed = prepare_for_training.prepare_for_training(data, normalize_data=normalize_data)[0]

        self.data = data_processed
        self.labels = labels.reshape(-1, 1)
        # input_layers = 28 * 28 * 1 = 728,hidden_layers = 25 or 100, out_layers = 10
        self.layers = layers
    ...
    def predict(self, data):
        ...
        data_processed = prepare_for_training.prepare_for_training(data, normalize_data=self.normalize_data)[0]
    ...
    @staticmethod
    def gradient_descent(data, labels, layers, unrolled_theta, alpha, regularization_param, max_iteration):
        # pylint: disable=too-many-arguments
    ...
    def gradient_step(data, labels, unrolled_thetas, layers, regularization_param):
        ...
        # Do backpropagation.
        thetas_rolled_gradient = MultilayerPerceptron.back_propagation(
            data, labels, thetas, layers, regularization_param
        )

        # Unrolled thetas gradients.
    ...
    def cost_function(data, labels, thetas, layers, regularization_param):
        ...
        # Get the size of output layer (num of labels).
        num_labels = layers[-1]
        ...
        for layer_index in range(num_layers - 1):
            ...
            theta_squares_sum += theta_squares_sum + np.sum(theta[:, 1:] ** 2)

        regularization = (regularization_param / (2 * num_examples)) * theta_squares_sum
    ...
    def feedforward_propagation(data, thetas, layers):
        ...
        for layer_index in range(num_layers - 1):
            theta = thetas[layer_index]
            out_layer_activation = sigmoid.sigmoid(in_layer_activation @ theta.T)
            # Add bias units.
    ...
    def back_propagation(data, labels, thetas, layers, regularization_param):
        ...
        # Get total number of possible output labels.
        ...
        for example_index in range(num_examples):
            ...
            for layer_index in range(num_layers - 1):
                ...
                layer_activation = np.vstack((np.array([[1]]), sigmoid.sigmoid(layer_input)))

                layers_input[layer_index + 1] = layer_input
            ...
            for layer_index in range(num_layers - 2, 0, -1):
                ...
                delta[layer_index] = (layer_theta.T @ next_delta) * sigmoid_gradient.sigmoid_gradient(layer_input)
        ...
        for layer_index in range(num_layers - 1):
            ...
            regularization = (regularization_param / num_label_types) * current_delta
    ...
    def thetas_roll(unrolled_thetas, layers):
        ...
        for layer_index in range(num_layers - 1):
            ...
            # We need to remember about bias unit.
